...
export function Dashboard({ clients, services, expenses, currentUser, onRefresh }: DashboardProps) {
  ...
  // Status removido do state visual, será fixo no submit
  ...
  const stats = useMemo(() => {
    ...
    // Restaurando contadores de status originais
    const activeServices = filteredServices.filter(s => s.status === 'IN_PROGRESS' || s.status === 'PENDING').length;
  ...
  const handleCreateService = async (e: React.FormEvent) => {
    ...
    const newService: ServiceRecord = {
        ...
        paid: isPaid,
        status: 'PENDING' // AQUI ESTÁ O SEGREDO: Fixo internamente
    };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Main Stats Grid - VISUAL ORIGINAL */}
