...
import { FileSpreadsheet, Building2, FolderOpen, ChevronRight, FileText, PieChart, CreditCard, Banknote, QrCode, Table, ShieldCheck, FileDown, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
import { ServiceDocumentModal } from './ClientDetails';
import { updateService } from '../services/storageService';
// @ts-ignore
...
export const Reports: React.FC<ReportsProps> = ({ clients, services, currentUser, onRefresh }) => {
    ...
    const stats = useMemo(() => {
        const revenue = filteredData.reduce((sum, s) => sum + s.cost, 0);
        const cost = filteredData.reduce((sum, s) => sum + (s.driverFee || 0), 0);
        const pending = filteredData.filter(s => !s.paid).reduce((sum, s) => sum + s.cost, 0);
                                    ...
                                    {filteredData.length === 0 ? (
                                        ...
                                        filteredData.map(s => (
                                            ...
                                            >
                                                ...
                                                <td className="p-3 text-center">
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); handleTogglePayment(s); }}
                                                        className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold transition-colors ${s.paid
