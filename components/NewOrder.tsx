...
export const NewOrder: React.FC<NewOrderProps> = ({ clients = [], onSave, onCancel }) => {
    ...
    const handleAddAddress = (t: 'pickup' | 'delivery') => {
        if (t === 'pickup') setPickupAddresses([...pickupAddresses, '']);
        else setDeliveryAddresses([...deliveryAddresses, '']);
    };

    const handleRemoveAddress = (t: 'pickup' | 'delivery', index: number) => {
        if (t === 'pickup') {
    ...
    const handleAddressChange = (t: 'pickup' | 'delivery', index: number, value: string) => {
        if (t === 'pickup') {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-slide-up">
            ...
            <form onSubmit={handleSubmit} className="bg-[#0f172a] rounded-xl shadow-2xl border border-slate-700 overflow-hidden text-slate-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-3 p-4 bg-blue-900/10 rounded-xl border border-blue-900/30">
                            ...
                            {pickupAddresses.map((addr, idx) => (
                                <div key={`pickup-${idx}`} className="flex gap-2 relative">
                        ...
                        <div className="space-y-3 p-4 bg-emerald-900/10 rounded-xl border border-emerald-900/30">
                            ...
                            {deliveryAddresses.map((addr, idx) => (
                                <div key={`delivery-${idx}`} className="flex gap-2 relative">
                    ...
                    <div className="pt-6 border-t border-slate-700">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                            <div>
                                ...
                                <div className="relative">
                                    <DollarSign size={18} className="absolute left-3 top-3 text-emerald-500" />
                                    <input required type="number" step="0.01" className="w-full pl-10 p-2.5 border border-slate-700 rounded-lg bg-[#1e293b] text-xl font-bold text-emerald-400 focus:border-emerald-500 outline-none" value={cost} onChange={e => setCost(e.target.value)} placeholder="0.00" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-red-400 mb-1">Pago ao Motoboy (R$)</label>
                                <div className="relative">
                                    <Bike size={18} className="absolute left-3 top-3 text-red-500" />
                                    <input required type="number" step="0.01" className="w-full pl-10 p-2.5 border border-slate-700 rounded-lg bg-[#1e293b] text-xl font-bold text-red-400 focus:border-red-500 outline-none" value={driverFee} onChange={e => setDriverFee(e.target.value)} placeholder="0.00" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <div className="relative">
                                    <Timer size={16} className="absolute left-3 top-3 text-slate-500" />
                        ...
                        <div className="p-4 bg-[#1e293b] rounded-lg flex justify-between items-center border border-slate-700 shadow-inner">
                            <div>
                                ...
                                <span className="text-2xl font-bold text-white">R$ {currentTotal.toFixed(2)}</span>
                            </div>
                ...
                {/* Footer do Form */}
                <div className="p-6 border-t border-slate-700 bg-[#1e293b] flex justify-end gap-3">
                    <button type="button" onClick={onCancel} className="px-6 py-3 text-slate-400 font-bold hover:text-white transition-colors">
                        Cancelar
                    </button>
